...
pr_files = dir(fullfile(to_pr_spei, 'pr_Amon*'));
...
for ii = 1:length(pr_files)
	...
	%% Obtain pr file path
	f_pr = strcat(to_pr_spei, curr_file_base, '.nc');
	disp(f_pr);

	%% Obtain spei file variable names
	sdi_varname=cell(1,1);
	sdi_varname{1} = 'spei_gamma_06';

	%% Obtain spei file paths
	f_spei = cell(1,1);
	f_spei{1} = strcat(to_spei, curr_file_base, '_', sdi_varname{1}, '.nc');
	
	%% Obtain output filenames
	f_spei_met=cell(1,1);
	f_spei_met{1} = strcat(to_spei_met, curr_file_base, '_', sdi_varname{1}, '_drought_metrics_numbered', '.nc');
	...
	%% Start loop to run the input files through the scripts

	idx=1;
    %%% create output netcdf file    
    out_file = f_spei_met{idx};
    ...
    sdi = ncread(f_spei{idx},sdi_varname{idx}); % missing value fill value -32767
    ...
    %% Create drought percentile arrays (thresholds based on normal distribution)
    ...
    %% Create drought duration, avg intensity, and prolonged drougth variables
    intensity = 'extreme';
