...
def transform_fitted_gamma(
    ...
    if (np.ma.is_masked(values) and values.mask.all()) or np.all(np.isnan(values)):
        #print('all values at this grid are nan or masked.. returning values')
        return values
    ...
    if (np.ma.is_masked(param_values) and param_values.mask.all()) or np.all(np.isnan(param_values)):
        #print('all param_values at this grid are nan or masked.. returning a 1032 length np.ndarray of NaNs')
        np_nan_vec = np.empty(values.shape)
        np_nan_vec[:] = np.NaN
        return np_nan_vec

    # validate (and possibly reshape) the input array
    param_values = _validate_array(param_values, periodicity)

    # find the percentage of zero values for each time step
    param_zeros = (param_values == 0).sum(axis=0)
    param_probabilities_of_zero = param_zeros / param_values.shape[0]

    # replace zeros with NaNs
    param_values[param_values == 0] = np.NaN
    ###############################################################
    ...
    if (alphas is None) or (betas is None):
        alphas, betas = \
            gamma_parameters(
                param_values, # Mine (changed from values)
                param_data_start_year, # Mine (changed from data_start_year)
                calibration_start_year,
                calibration_end_year,
                periodicity,
            )
