...
# ------------------------------------------------------------------------------
def _drop_param_data_into_shared_arrays_grid(dataset: xr.Dataset,
                                            var_names: list,
    ...
    for var_name in var_names:
        ...
        new_var_name = "param_" + var_name # Mine

        # add to the dictionary of arrays
        _global_shared_arrays[new_var_name] = {
...
def _compute_write_index(keyword_arguments):
    ...
    # Mine
    ################################################################
    if "netcdf_params_precip" in keyword_arguments: 
        param_files.append(keyword_arguments["netcdf_params_precip"])
    if "netcdf_params_pet" in keyword_arguments:
        param_files.append(keyword_arguments["netcdf_params_pet"])
    ################################################################
    ...
    param_dataset = xr.open_mfdataset(param_files, chunks=chunks) # Mine
    ...
    for var in dataset.data_vars:
        if var not in input_var_names:
            ...
            param_dataset = param_dataset.drop(var) # Mine

    # get the initial year of the data
    data_start_year = int(str(dataset["time"].values[0])[0:4])
    param_data_start_year = int(str(param_dataset["time"].values[0])[0:4]) # Mine
    keyword_arguments["data_start_year"] = data_start_year
    keyword_arguments["param_data_start_year"] = param_data_start_year # Mine
    ...
    if "var_name_precip" in keyword_arguments:
        ...
        # Mine
        #####################################################################
        param_precip_unit = param_dataset[precip_var_name].units.lower() 
        if param_precip_unit not in ("mm", "millimeters", "millimeter", "mm/dy"):
            if param_precip_unit in ("inches", "inch"):
                # inches to mm conversion (1 inch == 25.4 mm)
                param_dataset[precip_var_name].values *= 25.4
            else:
                raise ValueError(f"Unsupported precipitation units: {param_precip_unit}")
        #####################################################################
    ...
    if "var_name_pet" in keyword_arguments:
        ...
        # Mine
        #####################################################
        param_pet_unit = param_dataset[pet_var_name].units.lower()
        if param_pet_unit not in ("mm", "millimeters", "millimeter"):
            if param_pet_unit in ("inches", "inch"):
                # inches to mm conversion (1 inch == 25.4 mm)
                param_dataset[pet_var_name].values *= 25.4
            else:
                raise ValueError(f"Unsupported param PET units: {param_dataset[pet_var_name].units}")
        #####################################################
    ...
    if input_type == InputType.divisions:
        ...
        param_output_shape = \
            _drop_param_data_into_shared_arrays_grid(param_dataset,
                                                    input_var_names,
                                                    keyword_arguments["periodicity"],
                                                    keyword_arguments["param_data_start_year"])        
    # build an arguments dictionary appropriate to the index we'll compute
    args = _build_arguments(keyword_arguments)

    # Mine
    ###################################################################################
    if (keyword_arguments["index"] == "spi") or (keyword_arguments["index"] == "spei"):
        #args["netcdf_params_precip"] = \
        #        keyword_arguments["netcdf_params_precip"]
        #args["netcdf_params_pet"] = \
        #        keyword_arguments["netcdf_params_pet"]
        args["param_data_start_year"] = \
                keyword_arguments["param_data_start_year"]
    ##################################################################################
...
# ------------------------------------------------------------------------------
def _spi(precips, param_precips, parameters):

    return indices.spi(values=precips,
                       param_values=param_precips, # Mine
                       scale=parameters["scale"],
                       distribution=parameters["distribution"],
                       data_start_year=parameters["data_start_year"],
                       param_data_start_year=parameters["param_data_start_year"], # Mine
                       calibration_year_initial=parameters["calibration_year_initial"],
                       calibration_year_final=parameters["calibration_year_final"],
                       sub_array_start_index=parameters["sub_array_start"], # Mine
                       sub_array_end_index=parameters["sub_array_end"], # Mine
                       periodicity=parameters["periodicity"])
                       #save_params=parameters["save_params"], # Mine
                       #load_params=parameters["load_params"]) # Mine



# ------------------------------------------------------------------------------
def _spei(precips, pet_mm, param_precips, param_pet, parameters):

    return indices.spei(precips_mm=precips,
                        pet_mm=pet_mm,
                        param_precips_mm=param_precips, # Mine
                        param_pet_mm=param_pet, # Mine
                        scale=parameters["scale"],
                        distribution=parameters["distribution"],
                        data_start_year=parameters["data_start_year"],
                        param_data_start_year=parameters["param_data_start_year"], # Mine
...
def _apply_along_axis(params):
    ...
    # Mine
    ########################################################################
    args["sub_array_start"] = start_index
    args["sub_array_end"] = end_index
        

    if params["index"] == "spi":
        # PR
        param_var_name_pr = "param_" + params["input_var_name"]
        
        param_pr = _global_shared_arrays[param_var_name_pr][_KEY_ARRAY]
        param_pr_shape = _global_shared_arrays[param_var_name_pr][_KEY_SHAPE]
        
        np_param_pr = np.frombuffer(param_pr.get_obj()).reshape(param_pr_shape)
        param_pr_sub_array = np_param_pr[start_index:end_index]

        args["param_pr_sub_array"] = param_pr_sub_array
    ...
    for i, (x, param_x) in enumerate(zip(sub_array, param_pr_sub_array)):
        if params["input_type"] == InputType.grid:
            for j in range(x.shape[0]):
                temp_111 = func1d(x[j], param_x[j], parameters=args)
                computed_array[i, j] = temp_111
    ########################################################################
...
def _apply_along_axis_double(params):
    ...
    if params["index"] == "spei":
        # PR
        param_var_name_pr = "param_" + params["var_name_precip"]
        ...
        # PET
        param_var_name_pet = "param_" + params["var_name_pet"]

        param_pet = _global_shared_arrays[param_var_name_pet][_KEY_ARRAY]
        param_pet_shape = _global_shared_arrays[param_var_name_pet][_KEY_SHAPE]

        np_param_pet = np.frombuffer(param_pet.get_obj()).reshape(param_pet_shape)
        param_pet_sub_array = np_param_pet[start_index:end_index]
        #args["param_pet_sub_array"] = param_pet_sub_array
    ########################################################################
    ...
    param_x = np.empty(param_pr_sub_array.shape)
    param_y = np.empty(param_pet_sub_array.shape)

    for kk, (p_x, p_y) in enumerate(zip(param_pr_sub_array, param_pet_sub_array)):
        for ll in range(p_x.shape[0]):
            param_x[kk, ll] = p_x[ll] # vector of length 420
            param_y[kk, ll] = p_y[ll] # vector of length 420

    for i, (x, y) in enumerate(zip(sub_array_1, sub_array_2)):
        if params["input_type"] == InputType.grid:
            for j in range(x.shape[0]):
                if params["index"] == "pet":
                    computed_array[i, j] = func1d(x[j], y, parameters=args)
                else:
                    temp_11 = func1d(x[j], y[j], param_x[i, j], param_y[i, j], parameters=args) # Mine (added param_<x,y> arguments)
                    computed_array[i, j] = temp_11
        elif params["input_type"] == InputType.divisions:
            computed_array[i] = func1d(x, y, parameters=args)
        else:
...
def main():  # type: () -> None
    ...
    try:
        ...
        parser.add_argument(
            "--netcdf_params_precip", 
            help="NetCDF filename to compute gamma parameters",
        ) # Mine
        parser.add_argument(
            "--netcdf_params_pet", 
            help="NetCDF filename to compute gamma parameters",
        ...
        if arguments.index in ["spi", "scaled", "all"]:
            ...
            netcdf_params_precip = _prepare_file(arguments.netcdf_params_precip, # Mine
                                                 arguments.var_name_precip) # Mine
            ...
            for scale in arguments.scales:
                for dist in indices.Distribution:
                    ...
                    kwrgs = {"index": "spi",
                             ...
                             "netcdf_params_precip": netcdf_params_precip} # Mine
            ...
            if netcdf_params_precip != arguments.netcdf_params_precip: # Mine
                os.remove(netcdf_params_precip) # Mine
        ...
        if arguments.index in ["spei", "scaled", "all"]:
            ...
            # Mine
            ##################################################################################
            netcdf_params_precip = _prepare_file(
                arguments.netcdf_params_precip, arguments.var_name_precip
            )
            netcdf_params_pet = _prepare_file(arguments.netcdf_params_pet, arguments.var_name_pet)
            ##################################################################################
            ...
            for scale in arguments.scales:
                for dist in indices.Distribution:
                    ...
                    kwrgs = {
                        ...
                        "output_file_base": arguments.output_file_base,
                        "netcdf_params_precip": netcdf_params_precip, # Mine
                        "netcdf_params_pet": netcdf_params_pet} # Mine
            ...
            # Mine 
            ###########################################
            if netcdf_params_precip != arguments.netcdf_params_precip:
                os.remove(netcdf_params_precip)
            if netcdf_params_pet != arguments.netcdf_params_pet:
                os.remove(netcdf_params_pet)
            ###########################################
