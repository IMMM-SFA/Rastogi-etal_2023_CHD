...
def spi(
        ...
        data_start_year: int,
        param_data_start_year: int, # Mine
        calibration_year_initial: int,
        calibration_year_final: int,
        sub_array_start_index: int, # Mine
        sub_array_end_index: int, # Mine
        periodicity: compute.Periodicity,
        fitting_params: Dict = None,
        save_params: str = None, # Mine
        load_params: str = None, # Mine
) -> np.ndarray:
    """
    ...
    ############################################################################
    ############################################################################
    param_shape = param_values.shape
    if len(param_shape) == 2:
        param_values = param_values.flatten()
    elif len(param_shape) != 1:
        message = "Invalid shape of param input array: {param_shape}".format(param_shape=param_shape) + \
                  " -- only 1-D and 2-D arrays are supported" + "and precip shape is {shape}".format(shape=shape)
        _logger.error(message)
        raise ValueError(message)

    if (np.ma.is_masked(param_values) and param_values.mask.all()) or np.all(np.isnan(param_values)):
        np_nan_vec = np.empty(shape)
        np_nan_vec[:] = np.NaN
        return np_nan_vec

    if np.amin(param_values) < 0.0:
        _logger.warn("Input contains negative values -- all negatives clipped to zero")
        param_values = np.clip(param_values, a_min=0.0, a_max=None)

    param_original_length = param_values.size

    param_values = compute.sum_to_scale(param_values, scale)

    if periodicity is compute.Periodicity.monthly:
        param_values = utils.reshape_to_2d(param_values, 12)
    elif periodicity is compute.Periodicity.daily:
        param_values = utils.reshape_to_2d(param_values, 366)
    else:
        raise ValueError("Invalid periodicity argument: %s" % periodicity)
    ############################################################################
    ############################################################################
    ...
    if distribution is Distribution.gamma:
        ...
        values = compute.transform_fitted_gamma(
            values,
            param_values, # Mine
            param_data_start_year, # Mine
            data_start_year,
            calibration_year_initial,
            calibration_year_final,
            sub_array_start_index, # Mine
            sub_array_end_index, # Mine
            periodicity,
            alphas,
            betas,
            #save_params, # Mine
...
@numba.jit
def spei(
        precips_mm: np.ndarray, # (should be vector of length 1032)
        pet_mm: np.ndarray, # (should be vector of length 1032)
        param_precips_mm: np.ndarray, # Mine (should be vector of length 420)
        param_pet_mm: np.ndarray, # Mine (should be vector of length 420)
        scale: int,
        distribution: Distribution,
        periodicity: compute.Periodicity,
        data_start_year: int,
    ...
    # Mine
    ###############################################################
    if (np.ma.is_masked(param_precips_mm) and param_precips_mm.mask.all()) \
            or np.all(np.isnan(param_precips_mm)):
        return param_precips_mm

    if param_precips_mm.size != param_pet_mm.size:
        message = "Incompatible param_precipitation and PET arrays"
        _logger.error(message)
        raise ValueError(message)

    if np.amin(param_precips_mm) < 0.0:
        _logger.warn("param_Input contains negative values -- all negatives clipped to zero")
        param_precips_mm = np.clip(param_precips_mm, a_min=0.0, a_max=None)

    param_p_minus_pet = (param_precips_mm.flatten() - param_pet_mm.flatten()) + 1000.0
    param_original_length = param_precips_mm.size
    param_scaled_values = compute.sum_to_scale(param_p_minus_pet, scale)
    ###############################################################
    ...
    if distribution is Distribution.gamma:
        ...
        # Mine note that transform_fitted_values is a vector of length 1032
    elif distribution is Distribution.pearson:
