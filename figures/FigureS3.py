...
for rr in np.arange(np.shape(regions)[0]):
    ...
    indxrr[rr,:] = np.argsort(hm1[0,:,0])
    indx = indxrr[rr,:]

    hmnew[0:np.shape(gcm)[0],:] = hm1[rr,indx,0:5]
...
plt.savefig(f'FigureS3_{drght_indx}.pdf')
