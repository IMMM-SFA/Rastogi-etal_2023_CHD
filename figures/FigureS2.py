...
regions = ("southwest","southcentral","southeast","northwest","midwest","northcentral","northeast")
region = regions[0] #Change it for each region or loop through regions
#Read GCM name
...
for gg in np.arange(np.shape(gcm)[0]):
    ...
    fhis  = nc.Dataset(f'{dir}count_HW_t95_3days_{drght_indx}_{drght_ints}_drought_{drght_dur}_{gcm[gg]}_historical_{region}.nc')
    fssp  = nc.Dataset(f'{dir}count_HW_t95_3days_{drght_indx}_{drght_ints}_drought_{drght_dur}_{gcm[gg]}_ssp585_{region}.nc')
